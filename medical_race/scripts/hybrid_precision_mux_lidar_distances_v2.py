...
class HybridPrecisionMux:
    ...
    def _bedside_pid(self, now, current_yaw):
        ...
        front, left, right = self.latest_distances
        side_range = left if side == "left" else right
        if not (math.isfinite(front) and front > 0.0 and
                math.isfinite(side_range) and side_range > 0.0):
            rospy.logwarn_throttle(
                1.0,
                "Invalid selected lidar distances: front=%s, %s=%s "
                "(raw left=%s right=%s)",
                str(front), side, str(side_range), str(left), str(right))
            return None, False
        if front > self.max_docking_range or side_range > self.max_docking_range:
            rospy.logwarn_throttle(
                1.0,
                "Bedside PID target surface not found: front=%.2f, %s=%.2f",
                front, side, side_range)
            return None, False
        ...
        rospy.loginfo_throttle(
            0.5,
            "Bedside PID %s: front=%.3f err=%.3f, side=%.3f err=%.3f, "
    ...
    def _control(self, _event):
        ...
        with self.lock:
            ...
            if self.mode == "pid" and robot_yaw is not None:
                command, settled = self._bedside_pid(now, robot_yaw)
                if command is None:
                    self.precision_tolerance_since = None
                    self.cmd_pub.publish(Twist())
                    return
