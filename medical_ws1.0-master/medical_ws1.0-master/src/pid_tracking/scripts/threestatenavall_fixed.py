...
class ThreeStageNavigator:
    ...
    def __init__(self, tasktype=1):
        ...
        if self.lidar_distance:
            rospy.loginfo("从ROS参数加载lader_point距离数据 (保留原始值，不再在加载时减0.2)")
            # 使用参数文件中的数据更新精确距离
    ...
    def precise_position_single_axis(self, axis, target_distance, direction_name, timeout=45):
        """
        单轴精确定位 - 基于pid_node.py的稳定逻辑
        
        Args:
            axis (int): 轴向 (0=前方, 1=左侧, 2=右侧)
            target_distance (float): YAML中的目标距离
            direction_name (str): 方向名称（用于日志）
            timeout (float): 超时时间（秒）
        
        Returns:
            bool: 是否成功
        """
        # 设置PID控制状态（与pid_node.py一致的映射）
        self.node.distance_state = axis + 1
        
        # PID控制需要的目标值（在此统一减0.2m）
        pid_target = target_distance - 0.2
        pid_target = max(pid_target, 0.12)
        tolerance = 0.02  # 放宽容差到2cm
        dwell_required = 0.3  # 0.3秒驻留确认
        ok_since = None

        rospy.loginfo(f"     PID目标: {pid_target:.3f}m, 精度: {tolerance:.3f}m, 驻留: {dwell_required}s")

        start_time = rospy.Time.now()
        
        # 使用pid_node.py中验证的while循环逻辑
        while not rospy.is_shutdown():
            # 检查当前距离精度
            if axis >= len(self.node.distance):
                rospy.logerr(f"     距离传感器轴{axis}不存在")
                return False
            ...
            if error <= tolerance:
                ...
                if (rospy.Time.now() - ok_since).to_sec() >= dwell_required:
                    rospy.loginfo(f"     距离误差: {error:.4f}m ≤ {tolerance:.3f}m，且已驻留 {dwell_required}s ✅")
                    break
            else:
                ok_since = None
            ...
            if elapsed_time > timeout:
                rospy.logerr(f"     {direction_name}方向定位超时 ({timeout}s)")
                rospy.logerr(f"     最终误差: {error:.4f}m > {tolerance:.3f}m")
                self.node.publish_vel(0, 0, 0)
                return False
            
            # 调用pid_node.py的position_loop方法
        ...
        # 停止运动
        self.node.publish_vel(0, 0, 0)
        rospy.loginfo(f"     {direction_name}方向定位成功")
        return True
