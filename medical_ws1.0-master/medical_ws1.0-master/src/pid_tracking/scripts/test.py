...
class Race_task():
    ...
    def config(self):
        ...
        if self.choose_data == 1:
            with self.sm:
                ...
                smach.StateMachine.add('FRONT_POSITION_LOOP', PositionLoopState(self.node, self.lidar_distance[1][0], axis=0), transitions={'succeeded': 'SEND_1', 'in_progress': 'FRONT_POSITION_LOOP'})
                ...
                # 添加右定位的循环状态
                smach.StateMachine.add('RIGHT_POSITION_LOOP', PositionLoopState(self.node, self.lidar_distance[2][2], axis=2), transitions={'succeeded': 'TASK6', 'in_progress': 'RIGHT_POSITION_LOOP'})
                smach.StateMachine.add('TASK6', TaskState(self.node, 5), transitions={'succeeded': 'RIGHT_POSITION_LOOP2', 'failed': 'TASK_COMPLETED'})

                smach.StateMachine.add('RIGHT_POSITION_LOOP2', PositionLoopState(self.node, self.lidar_distance[3][2], axis=2), transitions={'succeeded': 'FRONT_POSITION_LOOP2', 'in_progress': 'RIGHT_POSITION_LOOP2'})
                smach.StateMachine.add('FRONT_POSITION_LOOP2', PositionLoopState(self.node, self.lidar_distance[3][0], axis=0), transitions={'succeeded': 'SEND_3', 'in_progress': 'FRONT_POSITION_LOOP2'})
            ...
            with self.sm:
                ...
                # 添加右定位的循环状态
                smach.StateMachine.add('RIGHT_POSITION_LOOP', PositionLoopState(self.node, self.lidar_distance[2][2], axis=2), transitions={'succeeded': 'TASK3', 'in_progress': 'RIGHT_POSITION_LOOP'})
                smach.StateMachine.add('TASK3', TaskState(self.node, 2), transitions={'succeeded': 'RIGHT_POSITION_LOOP1', 'failed': 'TASK_COMPLETED'})

                smach.StateMachine.add('RIGHT_POSITION_LOOP1', PositionLoopState(self.node, self.lidar_distance[3][2], axis=2), transitions={'succeeded': 'FRONT_POSITION_LOOP', 'in_progress': 'RIGHT_POSITION_LOOP1'})
                smach.StateMachine.add('FRONT_POSITION_LOOP', PositionLoopState(self.node, self.lidar_distance[3][0], axis=0), transitions={'succeeded': 'SEND_1', 'in_progress': 'FRONT_POSITION_LOOP'})
                ...
                smach.StateMachine.add('FRONT_POSITION_LOOP2', PositionLoopState(self.node, self.lidar_distance[1][0], axis=0), transitions={'succeeded': 'SEND_3', 'in_progress': 'FRONT_POSITION_LOOP2'})
    ...
    def test(self):
        ...
        try:
            with self.sm:
                    smach.StateMachine.add('SEND_1', Push_medicines_state(self._arm_serial, '1\r'),transitions={'succeeded': 'SEND_3', 'failed': 'TASK_COMPLETED'})
                    smach.StateMachine.add('SEND_3', Push_medicines_state(self._arm_serial, '3\r'),transitions={'succeeded': 'TASK_COMPLETED', 'failed': 'TASK_COMPLETED'})
                    self.sis = smach_ros.IntrospectionServer('server_name', self.sm, '/SM_ROOT')
            self.sis.start()
                    # 执行状态机
            outcome = self.sm.execute()
