...
void RobotCtrl::Plan(const ros::TimerEvent& event){
    ...
    {            
        if (plan_ ){
            ...
            if (GetEuclideanDistance(robot_pose,global_path_.poses.back())<= goal_dist_tolerance_
                ...
                cmd_vel.linear.x = 0;
                cmd_vel.linear.y = 0;
                cmd_vel.angular.z = 0;  // 医疗机器人到达目标后停止，不进行小陀螺
                cmd_vel.linear.z = 1;   // bool success or not
                cmd_vel_pub_.publish(cmd_vel);
                ROS_INFO("Medical Robot Planning Success - Target Reached!");
                return;
            }
            ...
            ROS_INFO("Medical Robot Planning takes %f ms and passed %d/%d.",
    ...
    else if(planner_state_ == 1)  // 原地旋转状态（如果需要的话）
    {
        geometry_msgs::Twist cmd_vel;
        cmd_vel.linear.x = 0;
        cmd_vel.linear.y = 0;
        cmd_vel.angular.z = set_yaw_speed_;  // 原地旋转
        cmd_vel.linear.z = 0;
        cmd_vel_pub_.publish(cmd_vel);
        ROS_INFO("Medical Robot - Rotating in place");
    }
...
void RobotCtrl::FollowTraj(const geometry_msgs::PoseStamped& robot_pose,
    ...
    printf("Medical Robot - diff_yaw: %f\n",diff_yaw);
    printf("Medical Robot - diff_distance: %f\n",diff_distance);
    ...
    // 医疗机器人在路径跟踪时不需要额外的角速度（除非转向需要）
    cmd_vel.angular.z = 0;  // 移除set_yaw_speed_，让机器人自然转向
...
void RobotCtrl::GlobalPathCallback(const nav_msgs::PathConstPtr & msg){
  if (!msg->poses.empty()){
      ...
      prune_index_ = 0;
      plan_ = true;
      ROS_INFO("Medical Robot - New global path received with %lu poses", msg->poses.size());
  }
}
...
void RobotCtrl::ImuCallback(const sensor_msgs::Imu &msg)
{
    ...
    // ROS_INFO("Medical Robot - IMU yaw: %f", yaw_);
}
...
bool RobotCtrl::Follower_StateReq(roborts_msgs::PidPlannerStatus::Request& req,
    ...
    ROS_INFO("Medical Robot - Request data: planner_state = %d, max_x_speed = %f, max_y_speed = %f, yaw_speed = %f"
    ...
    {
        ROS_ERROR("Medical Robot - Invalid planner state: %d (should be 0-2)", req.planner_state);
    ...
    if(req.max_x_speed > 0 && req.max_y_speed > 0)
    {
        ...
        ROS_INFO("Medical Robot - Speed updated: x=%.2f, y=%.2f", max_x_speed_, max_y_speed_);
    }
    
    if(req.yaw_speed >= 0){  // 允许0角速度
        set_yaw_speed_ = req.yaw_speed;
        ROS_INFO("Medical Robot - Yaw speed updated: %.2f", set_yaw_speed_);
    }
    ...
    switch(planner_state_) {
        case 0: state_name = "STOP"; break;
        case 1: state_name = "ROTATE"; break;
        case 2: state_name = "FOLLOW_PATH"; break;
        default: state_name = "UNKNOWN"; break;
    }
    ROS_INFO("Medical Robot - State changed to: %s", state_name.c_str());
...
{
  ros::init(argc, argv, "test_pid_position_follow");
  ROS_INFO("Starting Medical Robot PID Position Follow Controller...");
  RobotCtrl robotctrl;
  ros::spin();
  return 0;
}
