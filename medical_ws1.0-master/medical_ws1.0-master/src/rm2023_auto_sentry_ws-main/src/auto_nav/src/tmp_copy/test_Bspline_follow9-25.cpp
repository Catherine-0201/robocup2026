...
void RobotCtrl::FollowTraj(const geometry_msgs::PoseStamped& robot_pose,
    ...
    double e_yaw = NormalizeAngle(heading_ref - yaw_);  // 计算角度误差
    ...
    double angular_vel = 1.0 * yaw_err;  // 比例控制，可调Kp_yaw
    ...
    if (debug_count % 20 == 1) {  // 每2秒输出一次（10Hz * 2s = 20）
        ROS_INFO("目标检测调试 - 机器人位姿: (%.3f, %.3f), 目标位姿: (%.3f, %.3f), 距离: %.3f m, 容忍度: %.3f m", 
    ...
    if (dist_goal < goal_dist_tolerance_) {
        ...
        // 取消move_base的当前目标（参考threenav.py第一阶段停止逻辑）
