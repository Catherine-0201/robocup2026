...
RobotCtrl::RobotCtrl() {
    ...
    last_completed_goal_.pose.position.x = 0.0;
    last_completed_goal_.pose.position.y = 0.0;
    last_completed_goal_.pose.position.z = 0.0;
    ...
    target_done_pub_ = nh_.advertise<std_msgs::Int32>("/target_done", 10);  // 新添加
    
    // 初始化move_base客户端
    move_base_client_ = new actionlib::SimpleActionClient<move_base_msgs::MoveBaseAction>("move_base", true);
    ROS_INFO("Connecting to move_base server...");
    if (move_base_client_->waitForServer(ros::Duration(30.0))) {
        ROS_INFO("move_base server connected successfully");
    } else {
        ROS_WARN("move_base server connection timeout, cancel_goal may not work");
    }
}
...
void RobotCtrl::FollowTraj(const geometry_msgs::PoseStamped& robot_pose,
    ...
    // 添加自定义检测输出
    std::string status = (dist_goal < goal_dist_tolerance_) ? "停止" : "没有停止";
    ROS_INFO("(%.2f, %.2f)，(%.2f, %.2f)，%.2f米，%s",
             goal.pose.position.x, goal.pose.position.y,
             robot_pose.pose.position.x, robot_pose.pose.position.y,
             dist_goal, status.c_str());

    // 增强到达判断（参考PID）
    bool is_new_goal = true;
    double distance_to_last = GetEuclideanDistance(goal, last_completed_goal_);
    if (distance_to_last < goal_dist_tolerance_ * 2.0 && goal_just_completed_) {
        is_new_goal = false;
    }

    if ((dist_goal < goal_dist_tolerance_ || k == ref_times_.size() - 1) && is_new_goal) {
        // // ROS_INFO("=== 目标到达检测成功 ===");
        // // ROS_INFO("距离目标: %.3f m < 容忍度: %.3f m", dist_goal, goal_dist_tolerance_);
        
        // 停止机器人
        cmd_vel.linear.x = cmd_vel.linear.y = cmd_vel.angular.z = 0.0;
        // // ROS_INFO("速度命令已归零");
        
        // 添加/target_done发布（与PID一致）
        target_count_++;
        std_msgs::Int32 done_msg;
        done_msg.data = target_count_;
        target_done_pub_.publish(done_msg);
        ROS_INFO("B样条导航 - 医疗机器人已到达目标位置！完成目标数: %d", target_count_);
        
        // 修改取消逻辑：兼容threenav（不直接cancel，让threenav处理）
        plan_ = false;  // 状态重置
        ros::Duration(0.1).sleep();  // 给threenav时间处理
        // // 原有cancel注释掉，避免冲突
        // if (move_base_client_->isServerConnected()) {
        //     move_base_client_->cancelGoal();
        //     // ROS_INFO("move_base目标取消命令已发送 - B样条导航完成");
        // } else {
        //     // ROS_WARN("move_base服务器未连接，无法取消目标");
        // }
        
        // // ROS_INFO("=== 目标到达处理完成 ===");
        
        // 更新重复保护
        last_completed_goal_ = goal;
...
void RobotCtrl::ImuCallback(const sensor_msgs::Imu& msg) {
    ...
    if (!first_imu_received) {
        ...
        // ROS_INFO("记录初始IMU姿态: yaw=%.3frad (%.1f°)", initial_yaw_, initial_yaw_ * 180.0 / M_PI);
    }
}
...
void RobotCtrl::GetRobotPose(geometry_msgs::PoseStamped& robot_pose) {
    ...
    try {
        ...
        if (tf_success_count % 50 == 1) {  // 每5秒输出一次（10Hz * 5s = 50）
            ROS_INFO("TF获取成功 - 机器人位姿: x=%.3f, y=%.3f (来自tf), yaw=%.3f° (来自IMU)", 
