...
void RobotCtrl::FollowTraj(const geometry_msgs::PoseStamped& robot_pose,
    ...
    if (dist_goal < goal_dist_tolerance_) {
        ...
        // 新增：目标完成计数和发布（带重复保护）
        geometry_msgs::PoseStamped current_goal = traj.poses.back();
        ...
        if (distance_to_last_goal > goal_dist_tolerance_ * 2.0 || target_count_ == 0) {
            ...
            ROS_INFO("B-spline controller - Robot has reached target position! Completed targets: %d", target_count_);
        } else {
            ROS_DEBUG("Target repeated, skip counting - Distance to last target: %.3f", distance_to_last_goal);
        }
