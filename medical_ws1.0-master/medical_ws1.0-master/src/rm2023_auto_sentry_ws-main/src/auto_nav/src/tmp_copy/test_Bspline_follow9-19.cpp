...
void RobotCtrl::FollowTraj(const geometry_msgs::PoseStamped& robot_pose,
    ...
    if (dist_goal < goal_dist_tolerance_) {
        ...
        ROS_INFO("Target reached! Distance to goal: %.3f m (tolerance: %.3f m)", dist_goal, goal_dist_tolerance_);
    }
...
void RobotCtrl::GetRobotPose(geometry_msgs::PoseStamped& robot_pose) {
    // 这里需要根据实际的TF树结构来获取机器人位置
    // 暂时使用一个简单的实现，实际使用时需要根据具体情况调整
    robot_pose.header.frame_id = "map";
    robot_pose.header.stamp = ros::Time::now();
    robot_pose.pose.position.x = 0.0;  // 需要从TF或里程计获取
    robot_pose.pose.position.y = 0.0;  // 需要从TF或里程计获取
