...
RobotCtrl::RobotCtrl() {
    ...
    completed_targets_ = 0;  // 初始化目标完成计数
    ...
    target_done_pub_ = nh_.advertise<std_msgs::Int32>("/target_done", 10);  // 发布目标完成信号
    ...
    ROS_INFO("B样条跟踪器初始化完成，已连接到三段式导航系统");
}
...
void RobotCtrl::FollowTraj(const geometry_msgs::PoseStamped& robot_pose,
    ...
    if (dist_goal < goal_dist_tolerance_) {
        ...
        target_done_msg.data = completed_targets_;
        target_done_pub_.publish(target_done_msg);
        ROS_INFO("已发布目标完成信号到 /target_done 话题，完成计数: %d", completed_targets_);
        
        ROS_INFO("=== 目标到达处理完成，等待三段式导航系统响应 ===");
    }
