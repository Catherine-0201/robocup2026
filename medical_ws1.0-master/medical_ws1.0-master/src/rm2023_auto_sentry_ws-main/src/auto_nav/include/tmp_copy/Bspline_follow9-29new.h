...
private:
    ...
    double yaw_pid_;                    // yaw角度环PID参数
            double yaw_;                        // 机器人当前朝向
            double initial_yaw_;                // 机器人上电时的初始姿态
            double goal_dist_tolerance_;        // 目标距离容忍度
    bool plan_;                         // 是否启用路径规划的标志
    bool has_global_path_;              // 是否有全局路径

    // 新添加：用于目标到达和/target_done
    int target_count_;                  // 目标完成计数
    geometry_msgs::PoseStamped last_completed_goal_;  // 上次完成的目标
    bool goal_just_completed_;          // 是否刚完成目标（重复保护）
    ...
    void GetRobotPose(geometry_msgs::PoseStamped& robot_pose);
            ...
            ros::Publisher target_done_pub_;     // 新添加：发布/target_done
