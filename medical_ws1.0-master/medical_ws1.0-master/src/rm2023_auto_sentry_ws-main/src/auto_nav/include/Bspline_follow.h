...
private:
    ...
    int target_done_;                   // 目标完成计数，初值0
    geometry_msgs::PoseStamped former_goal_;  // 前一个目标，初值(0,0)
    int target_complete_;               // 目标完成状态，初值-1
    ...
    void GetRobotPose(geometry_msgs::PoseStamped& robot_pose);
            ...
            ros::Publisher target_done_pub_;     // 发布/target_done
