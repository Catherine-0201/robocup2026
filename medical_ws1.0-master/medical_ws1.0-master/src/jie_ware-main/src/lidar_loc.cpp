...
// 定位监控相关变量
bool localization_reliable = true;  // 定位是否可靠
ros::Time last_status_update;

// 定位状态回调函数
void localizationStatusCallback(const std_msgs::Bool::ConstPtr& msg)
{
    localization_reliable = msg->data;
    last_status_update = ros::Time::now();
    
    if (localization_reliable) {
        ROS_INFO("定位监控: 激光雷达定位恢复正常");
    } else {
        ROS_WARN("定位监控: 检测到定位异常，监控节点接管TF发布");
    }
}
...
void crop_map()
{
    if (map_msg.data.empty() || map_msg.info.width <= 0 || map_msg.info.height <= 0) {
        ROS_ERROR("Invalid map data: width=%d, height=%d, data_size=%zu",
                  map_msg.info.width, map_msg.info.height, map_msg.data.size());
        return;
    }

    cv::Mat map_raw(map_msg.info.height, map_msg.info.width, CV_8UC1, cv::Scalar(128));
    int xMax = map_msg.info.width / 2, xMin = xMax;
    int yMax = map_msg.info.height / 2, yMin = yMax;
    ...
    for (int y = 0; y < map_msg.info.height; y++) {
        for (int x = 0; x < map_msg.info.width; x++) {
            ...
            if (index >= map_msg.data.size()) {
                ROS_ERROR("Index out of bounds: index=%d, data_size=%zu", index, map_msg.data.size());
                return;
            }
    ...
    int new_half_width = abs(xMax - xMin) / 2 + 25;
    int new_half_height = abs(yMax - yMin) / 2 + 25;
    ...
    if (new_origin_x + new_width > map_msg.info.width)
        new_width = map_msg.info.width - new_origin_x;
    if (new_origin_y + new_height > map_msg.info.height)
        new_height = map_msg.info.height - new_origin_y;

    if (new_width <= 0 || new_height <= 0) {
        ROS_ERROR("Invalid ROI: width=%d, height=%d", new_width, new_height);
        return;
    }
    ...
    ROS_INFO("Cropped map: rows=%d, cols=%d", map_cropped.rows, map_cropped.cols);
}
...
{
    if (map_cropped.empty()) {
        ROS_ERROR("map_cropped is empty");
        return;
    }

    // map_temp = cv::Mat::zeros(map_cropped.size(), CV_8UC1);
    map_temp = map_cropped.clone();
    cv::Mat gradient_mask = createGradientMask(51);

    for (int y = 0; y < map_cropped.rows; y++) {
        ROS_INFO("processMap 节点启动M1");
        for (int x = 0; x < map_cropped.cols; x++) {
        
            if (map_cropped.at<uchar>(y, x) == 100) {
                int left = std::max(0, x - 25);
                int top = std::max(0, y - 25);
                int right = std::min(map_cropped.cols - 1, x + 25);
                int bottom = std::min(map_cropped.rows - 1, y + 25);
                cv::Rect roi(left, top, right - left + 1, bottom - top + 1);

                int mask_left = std::max(0, 25 - (x - left));
                int mask_top = std::max(0, 25 - (y - top));
                int mask_width = std::min(roi.width, gradient_mask.cols - mask_left);
                int mask_height = std::min(roi.height, gradient_mask.rows - mask_top);

                if (mask_width <= 0 || mask_height <= 0) {
                    ROS_ERROR("Invalid mask ROI at (%d, %d): width=%d, height=%d", x, y, mask_width, mask_height);
                    continue;
                }

                cv::Rect mask_roi(mask_left, mask_top, mask_width, mask_height);
...
void pose_tf()
{
    ...
    // 检查定位是否可靠，如果监控节点接管了TF发布，则停止发布
    if (!localization_reliable) {
        // 监控节点正在发布备用TF，避免冲突
        return;
    }
    
    // 检查监控状态更新是否超时 (超过5秒未收到状态更新，假设监控节点失效)
    if (!last_status_update.isZero() && 
        (ros::Time::now() - last_status_update).toSec() > 5.0) {
        ROS_WARN_THROTTLE(2.0, "监控节点状态超时，恢复激光雷达定位TF发布");
        localization_reliable = true;  // 重置为可靠状态
    }
    ...
    // 1.3 处理yaw角度。调整：去掉负号以匹配新的机器人正方向（机器人已旋转180度）
...
{
    setlocale(LC_ALL,"");                     //! 设置文本编码解码方式
    ros::init(argc, argv, "lidar_loc");       //! 初始化ROS节点 lidar_loc

    // 读取参数
    ros::NodeHandle private_nh("~");     
    private_nh.param<std::string>("base_frame", base_frame, "base_footprint");   //! param键， 要存储到的变量， 默认 
    ...
    // 订阅定位监控状态，实现协调工作
    ros::Subscriber status_sub = nh.subscribe("localization_status", 1, localizationStatusCallback);
