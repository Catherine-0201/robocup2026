...
void processMap()
{
    ...
    cv::Mat gradient_mask = createGradientMask(41);
    ...
    for (int y = 0; y < map_cropped.rows; y++) {
        ...
        for (int x = 0; x < map_cropped.cols; x++) {
            ...
            if (map_cropped.at<uchar>(y, x) == 100) {
                int left = std::max(0, x - 20);
                int top = std::max(0, y - 20);
                int right = std::min(map_cropped.cols - 1, x + 20);
                int bottom = std::min(map_cropped.rows - 1, y + 20);
                cv::Rect roi(left, top, right - left + 1, bottom - top + 1);

                int mask_left = std::max(0, 20 - (x - left));
                int mask_top = std::max(0, 20 - (y - top));
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "lidar_loc_copy");       //! 初始化ROS节点 lidar_loc
